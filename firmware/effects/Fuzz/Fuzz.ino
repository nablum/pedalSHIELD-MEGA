...
/* Fuzz.ino hard clips the input signal. */
...
ISR(TIMER4_CAPT_vect) 
{
  ...
  { 
    ...
    counter=0;
    if (!digitalRead(PUSHBUTTON_2)) if (threshold<THRESHOLD_MAX) threshold=threshold+THRESHOLD_STEP;
    if (!digitalRead(PUSHBUTTON_1)) if (threshold>THRESHOLD_MIN) threshold=threshold-THRESHOLD_STEP;
  ...
  //output is clipped for the threshold value
  if(output>threshold) output=32768;
  else if(output<-threshold) output=-32768;
